...
public:
    gtimer();
    gtimer(TIM_TypeDef *TIMx, uint32_t PSC, uint32_t ARR, uint16_t mode=0);
    ~gtimer();

    void set(TIM_TypeDef *TIMx, uint32_t PSC, uint32_t ARR, uint16_t mode=0);
    void start();
    void stop();
...
class gtimer_int : public gtimer {
public:
    gtimer_int();
